...
Overview:
This script takes the sentence-level processed data (including sentiment scores)
generated by `sec_ingestion.py` and engineers more advanced NLP features. It
calculates sentence embeddings, performs topic modeling using BERTopic, computes
sentence novelty compared to the previous year, and finally aggregates all these
features to the filing level (one row per company per year). The resulting
feature set is saved for downstream analysis or modeling tasks.
...
1.  **Load Processed Data:**
    - Reads sentence-level `.parquet` files generated by `sec_ingestion.py`
      for specified tickers and years.
2.  **Sentence Embeddings:**
    - Generates dense vector embeddings for each sentence using a SentenceTransformer
      model (e.g., `all-MiniLM-L6-v2`).
    - Includes optional caching (`joblib`) to speed up subsequent runs.
3.  **Novelty Calculation:**
    - Compares each sentence's embedding to the embeddings from the *previous year's*
      filing for the same company.
    - Calculates a novelty score (1 - max cosine similarity) indicating semantic difference.
4.  **Topic Modeling:**
    - Applies BERTopic clustering to the sentence embeddings across all loaded data.
    - Assigns a topic ID to each sentence and saves the trained topic model.
5.  **Feature Aggregation:**
    - Aggregates all sentence-level features (original sentiment/keywords, plus
      new embeddings-derived features like novelty and topic ID) to the filing level
      (one row per ticker per year).
    - Calculates statistics like means, counts, percentages, and topic distributions per filing.
6.  **Save Features & Models:**
    - Saves the final aggregated filing-level features to a `.parquet` file.
    - Saves the trained BERTopic model and topic information (e.g., keywords per topic).
...
def load_processed_data(ticker: str, years: List[int], data_dir: Path) -> Optional[pd.DataFrame]:
    """
    Loads and concatenates processed sentence-level parquet files for a specific
    ticker across a list of years. Selects only essential columns.

    Args:
        ticker: The company ticker symbol (e.g., "NVDA").
        years: A list of years to load data for.
        data_dir: The base directory containing the processed data (e.g., .../data/processed/sec_filings).

    Returns:
        A pandas DataFrame containing the combined data for the ticker and years,
...
def _encode_sentences_cached(sentences: Tuple[str], model_name: str, batch_size: int, device: str) -> Optional[np.ndarray]:
    """
    Internal helper function to generate sentence embeddings using SentenceTransformer.
    This function's results are intended to be cached by joblib.

    Args:
        sentences: A tuple of sentences to embed (must be tuple for joblib hashing).
        model_name: The name of the SentenceTransformer model.
        batch_size: Batch size for encoding.
        device: Device ('cpu' or 'cuda') to use for computation.
...
def run_bertopic_clustering(docs: List[str], embeddings: np.ndarray, model_save_dir: Path) -> Tuple[Optional[Any], Optional[np.ndarray]]:
    """
    Performs topic modeling using BERTopic on a list of documents and their
    pre-computed embeddings. Saves the trained BERTopic model.

    Args:
        docs: A list of text documents (sentences in this case).
        embeddings: A numpy array of pre-computed embeddings corresponding to the docs.
        model_save_dir: The directory path to save the trained BERTopic model.

    Returns:
        A tuple containing:
            - The trained BERTopic model object (or None on failure).
            - A numpy array of topic IDs assigned to each document (or None on failure).
    """
...
def calculate_novelty(current_embeddings: np.ndarray, previous_embeddings: np.ndarray) -> Optional[np.ndarray]:
    """
    Calculates a novelty score for each embedding in the current set compared
    to the embeddings from a previous set (e.g., previous year).

    The novelty score is defined as 1 minus the maximum cosine similarity
    between a current embedding and *any* embedding in the previous set.

    Args:
        current_embeddings: Numpy array of embeddings for the current period.
        previous_embeddings: Numpy array of embeddings for the previous period.

    Returns:
        A numpy array of novelty scores (float32, between 0 and 1) for each embedding
        in `current_embeddings`, or None if an error occurs or inputs are invalid.
    """
...
def aggregate_features_to_filing(df: pd.DataFrame) -> Optional[pd.DataFrame]:
    """
    Aggregates sentence-level features (sentiment, keywords, novelty, topics, etc.)
    up to the filing level (one row per ticker per year).

    Calculates various statistics like counts, means, standard deviations, percentages,
    and topic distributions for each filing.

    Args:
        df: A pandas DataFrame containing sentence-level data with NLP features.

    Returns:
        A pandas DataFrame with aggregated features at the filing level, or None if an error occurs.
    """
...
def main():
    """
    Main orchestration function for the NLP feature engineering pipeline.

    Steps:
    1. Loads processed sentence-level data for specified tickers and years.
    2. Generates sentence embeddings using a SentenceTransformer model.
    3. Calculates novelty scores for sentences compared to the previous year.
    4. Performs topic modeling using BERTopic on all sentences.
    5. Aggregates all sentence-level features (original and newly engineered) to the filing level.
    6. Saves the final aggregated features DataFrame and topic model information.
    """
